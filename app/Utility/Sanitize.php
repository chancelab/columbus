...
/**
 * Strips the specified tag attributes from output. First parameter is string from
 * where to remove tag attributes. All subsequent parameters are tag attributes.
 *
 * Ex.`$clean = Sanitize::stripTagAttributes($dirty, 'onclick');`
 *
 * Will remove all `<onclick>` tag attributes from the $dirty string.
 *
 * @param string $str,... String to sanitize
 * @return string sanitized String
 */
	public static function stripTagAttributes($str) {
		$params = func_get_args();

		for ($i = 1, $count = count($params); $i < $count; $i++) {
			// FIXME: 該当のタグ以降すべての属性を消してしまうので必要があれば修正する
			$str = preg_replace('/' . $params[$i] . '.([\s\S].*).(\'|\"|\()*(\'|\"|\))/i', '', $str);
		}
		return $str;
	}
