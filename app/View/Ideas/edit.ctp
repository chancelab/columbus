...
<?php echo $this->Form->create('Idea', array('type' => 'file') ); ?>
	<fieldset>
	...
	<?php
		...
		foreach ($addInputs as $addInput) {
			echo $this->Form->input("IdeaAddInput.$counter.id", array('type' => 'hidden'));
			echo $this->Form->input("IdeaAddInput.$counter.idea_id", array('type' => 'hidden'));
		...
		if (!empty($attachments)) {
			...
			foreach ($attachments as $file) {
				...
				echo $this->Form->input("DeletedFile.".$file['Attachment']['id'].".Delete", array('type' => 'checkbox', 'label' => $file['Attachment']['name']));
